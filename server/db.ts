...
/**
 * Deletes all products whose category name is NOT in the curated set, plus
 * orphaned categories. Returns counts of deleted products and categories.
 */
export async function purgeLegacyProducts(curatedNames: string[]) {
  const conn = await getRawConnection();
  if (!conn) return { deletedProducts: 0, deletedCategories: 0 };
  try {
    const placeholders = curatedNames.map(() => "?").join(",");
    const [rows] = await conn.execute(
      `SELECT id, name FROM categories WHERE name NOT IN (${placeholders})`,
      curatedNames
    );
    const legacyIds = (rows as any[]).map((r: any) => r.id);
    let deletedProducts = 0;
    if (legacyIds.length) {
      const p = legacyIds.map(() => "?").join(",");
      const [r] = await conn.execute(
        `DELETE FROM products WHERE categoryId IN (${p})`,
        legacyIds
      );
      deletedProducts = (r as any).affectedRows ?? 0;
      // Also remove products that have no category at all (uncategorized)
      const [solo] = await conn.execute(
        "DELETE FROM products WHERE categoryId IS NULL"
      );
      deletedProducts += (solo as any).affectedRows ?? 0;
    }
    const [catRes] = await conn.execute(
      `DELETE FROM categories WHERE name NOT IN (${placeholders})`,
      curatedNames
    );
    const deletedCategories = (catRes as any).affectedRows ?? 0;
