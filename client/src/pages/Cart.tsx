...
export default function Cart() {
  ...
  const buildPaymentMessage = () => {
    ...
    const itemLines = placedOrder.items
      .map(i => `  - ${i.title} x ${i.quantity} (${formatPrice(i.unitPriceCents * i.quantity)})`)
      .join("\n");
...
Please send me the payment instructions for ${placedOrder.paymentMethod}. Thank you!`;
      ...
      <StoreLayout>
        <div className="container py-8 md:py-12 max-w-lg mx-auto">
          ...
          <div className="bg-white border border-border rounded-lg p-5 md:p-6">
            <h2 className="font-display text-base font-bold text-center mb-2">Complete Your Payment</h2>
            <p className="text-center text-muted-foreground text-xs mb-6">
              Choose a contact method. We'll send you payment instructions.
            </p>

            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={openEmail}
                className="group flex flex-col items-center gap-2.5 border-2 border-border rounded-lg p-4 hover:border-primary hover:bg-primary/5 transition-all"
              >
                ...
                <div className="text-center">
                  <div className="font-bold text-xs mb-0.5">Email</div>
                  <div className="text-[10px] text-muted-foreground break-all leading-tight">{contactEmail || "Not set"}</div>
                </div>
              </button>

              <button
                onClick={openWhatsApp}
                className="group flex flex-col items-center gap-2.5 border-2 border-border rounded-lg p-4 hover:border-green-600 hover:bg-green-50 transition-all"
              >
                ...
                <div className="text-center">
                  <div className="font-bold text-xs mb-0.5">WhatsApp</div>
                  <div className="text-[10px] text-muted-foreground">{whatsappNumber || "Not set"}</div>
                </div>
              </button>
            </div>
    ...
    return (
      <StoreLayout>
        <div className="container py-20 flex flex-col items-center text-center">
          ...
          <h1 className="font-display text-xl font-black mb-1">Your cart is empty</h1>
          <p className="text-muted-foreground text-sm mb-5">Browse the shop and add some products to get started.</p>
          <Link href="/shop">
            <Button className="uppercase font-bold tracking-wide text-sm">Browse Shop</Button>
          </Link>
        </div>
      </StoreLayout>
    );
  ...
  return (
    <StoreLayout>
      <div className="px-4 md:container py-6 md:py-10">
        <h1 className="font-display text-xl md:text-2xl font-black uppercase tracking-wide mb-5 md:mb-6">
          Shopping Cart
        </h1>

        {/* Cart items */}
        <div className="bg-white border border-border rounded-md divide-y divide-border mb-5 md:mb-6">
          {items.map(item => (
            <div key={item.productId} className="flex gap-3 p-3 sm:p-4 items-start sm:items-center">
              <div className="h-14 w-14 sm:h-16 sm:w-16 rounded bg-muted/50 overflow-hidden flex items-center justify-center shrink-0">
                {item.imageUrl ? (
              ...
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-sm text-primary leading-snug truncate mb-0.5">{item.title}</div>
                <div className="text-[11px] text-muted-foreground mb-1.5">{formatPrice(item.priceCents)} each</div>
        ...
        {/* Summary */}
        <div className="bg-white border border-border rounded-md p-4 sm:p-5">
          <h2 className="font-display font-black uppercase tracking-wide text-sm mb-3">Order Summary</h2>
          ...
          <div className="border-t border-border pt-3 flex justify-between items-center mb-3">
            ...
            <span className="font-display font-black text-xl text-primary">{formatPrice(totalCents)}</span>
          </div>

          {belowMinimum && (
            <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 text-amber-800 rounded p-2.5 text-xs mb-3">
              <AlertCircle className="h-3.5 w-3.5 shrink-0 mt-0.5" />
              <span>
                Minimum order is <strong>{formatPrice(minimumOrderCents)}</strong>. Add{" "}
                <strong>{formatPrice(minimumOrderCents - totalCents)}</strong> more.
              </span>
            </div>
          )}
          ...
          <Button
            ...
            className="w-full h-11 font-bold uppercase tracking-wide text-sm"
          >
      ...
      <Dialog open={step === "checkout"} onOpenChange={open => { if (!open) setStep("cart"); }}>
        <DialogContent className="max-h-[90vh] overflow-y-auto w-full sm:max-w-md p-0 gap-0">
          <div className="p-4 sm:p-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div className="space-y-1">
                ...
                <Input id="co-name" value={name} onChange={e => setName(e.target.value)} placeholder="Jane Doe" className="h-9 text-sm" />
              </div>
              <div className="space-y-1">
                <Label htmlFor="co-email" className="text-xs">Email *</Label>
                <Input id="co-email" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="jane@example.com" className="h-9 text-sm" />
              </div>
            </div>
            <div className="space-y-1">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {settings?.paymentMethods?.map(method => (
                  <button
                    ...
                    className={`flex items-center gap-2 p-2 rounded-lg border-2 text-left text-xs font-medium transition-all ${
                  ...
                  >
                    ...
                    <span className="flex-1 truncate leading-tight">{method}</span>
                    {selectedPayment === method && <Check className="h-3 w-3 text-primary shrink-0" />}
                  </button>
                ))}
              </div>
            </div>

            <div className="flex justify-between text-xs font-bold border-t border-border pt-2.5">
              <span>Total to pay</span>
              <span className="text-primary">{formatPrice(totalCents)}</span>
            </div>
          </div>

          <div className="border-t border-border p-4 sm:p-5 flex flex-col-reverse sm:flex-row sm:justify-end gap-2">
            <Button variant="outline" size="sm" onClick={() => setStep("cart")} className="h-9 text-xs">
              <ArrowLeft className="h-3.5 w-3.5 mr-1.5" />
              Back
            </Button>
            <Button size="sm" onClick={handlePlaceOrder} disabled={placeOrder.isPending || !selectedPayment} className="h-9 text-xs font-bold">
              {placeOrder.isPending && <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" />}
